...
# (1) 게임 시작 시 커스터마이징 오버레이가 보이고 튜토리얼은 아직 가려져
# 있는지, (2) 색 선택 키(2: 빨강)를 누르면 플레이어 스프라이트 텍스처가
# 해당 색으로 바뀌고 오버레이가 사라지는지, (3) 튜토리얼 오버레이로 자연스럽게
# 넘어가는지 확인한다.
...
func _initialize() -> void:
	...
	if not customization_overlay.visible:
		push_error("FAIL: 시작 시 커스터마이징 오버레이가 보이지 않음")
		ok = false
