...
# slot_appearance 등 메모리 상태가 그대로 남아있어 디스크 저장을 우회해도
# 통과할 수 있으므로, 인스턴스를 아예 새로 만들어야 디스크에서 실제로
# 읽어오는지 검증된다).
...
func _initialize() -> void:
	...
	# 1단계: 새 슬롯 1을 골라 빨강 피부/검정 눈/짧은 머리로 커스터마이징하고,
	# 나무를 채집해 인벤토리도 채운 뒤 저장 파일이 실제로 생기는지 확인한다.
	...
	if player2.skin_color != expected_red:
		push_error("FAIL: 저장된 슬롯 1의 피부색이 복원되지 않음 (실제: %s)" % [player2.skin_color])
		ok = false
	if player2.hair_type != "short":
		push_error("FAIL: 저장된 슬롯 1의 머리종류가 복원되지 않음 (실제: %s)" % player2.hair_type)
		ok = false
